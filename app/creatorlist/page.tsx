...
useEffect(() => {
  ...
  return (
    <main className="px-10 py-14">
      ...
      <div className="mt-4">
        {(newCreators && newCreators.length > 0) ? newCreators.map((creator, index) => (
          <Link key={index} href={`/creators/${creator.walletAddress}`} onClick={() => setMyInteger(index)}>
            <div className="bg-white p-4 mb-4 rounded shadow flex items-center justify-between"> {/* Added justify-between */}
            <div className="flex items-center"> {/* Added flex and items-center */}
                ...
                <div>
                <h2 className="text-xl font-semibold text-black">{creatorNames[index]}</h2>
